...
audio_results = audio_chunk_view.order_by(audio_sim, asc=False).limit(2).select(audio_chunk_view.text, similarity=audio_sim).collect()

frame_view = memory.frame_views['video']
frame_sim = frame_view.frame_description.similarity(query_visual)
visual_results = frame_view.order_by(frame_sim, asc=False).limit(2).select(frame_view.frame, frame_view.frame_description, similarity=frame_sim).collect()
