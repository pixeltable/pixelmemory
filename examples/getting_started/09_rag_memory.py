...
document_chunks = [
    {
        ...
        "metadata": {"section": "List Comprehensions", "page": 45, "difficulty": "intermediate"},
    ...
    },
    {
        ...
        "metadata": {"section": "REST Principles", "version": "v2.1", "category": "architecture"},
...
filtered_context = (
    mem.where((mem.document_type == "documentation") | (mem.document_type == "guidelines"))
    .order_by(doc_similarity, asc=False)
    .select(
        mem.content,
        mem.document_id,
        mem.document_type,
        relevance=doc_similarity
    )
    .limit(2)
    .collect()
)
...
conversation_history = ["How do I handle errors in Python?", "What about list comprehensions?"]
...
def hybrid_rag_retrieval(query, boost_recent=True, top_k=3):
    ...
    hybrid_context = (
        ...
        .order_by(content_sim + (retrieval_boost * 0.1 if boost_recent else 0), asc=False)
        .limit(top_k)
        .collect()
    )
