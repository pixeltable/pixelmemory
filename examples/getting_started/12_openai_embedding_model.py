...
for model, results in comparison_results.items():
    if "error" not in results:
        print(
            f"{model}: {results['dimensions']} dims, score: {results['top_score']:.4f}"
        )
    else:
...
all_docs_result = memory_openai.select(
    memory_openai.doc_id, memory_openai.content, memory_openai.category
).collect()
...
for i, doc1 in enumerate(all_docs):
    for j in range(i + 1, len(all_docs)):
        ...
        sim_score = (
            ...
            .select(similarity=memory_openai.content.similarity(doc1["content"]))
            .collect()
        )
