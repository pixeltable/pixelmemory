...
complex_query = (
    mem.where(
        (mem.priority == "high") |
        (
            (mem.content.contains("performance")) &
            (mem.timestamp >= last_24h)
        )
    )
...
search_results = temporal_semantic_search("code review tasks", hours_back=48, min_priority="medium")
