...
## 🏗️ Built on Pixeltable's Foundation

Pixelmemory leverages Pixeltable's powerful declarative data infrastructure to provide memory capabilities:

- **Persistent Storage**: Uses Pixeltable's built-in persistence (no additional database setup)
- **Computed Columns**: Automatic embedding generation and vision analysis  
- **Semantic Search**: Built-in vector indexes for similarity search
- **Multimodal Support**: Unified interface for text, images, videos, audio, and documents
- **Incremental Updates**: Only recomputes what's changed, saving time and cost

**This is a reference implementation** showing how to build your own memory layer using Pixeltable's primitives. You can extend, modify, or use it as inspiration for your own agent memory architecture.
...
- ✅ `01_basic_memory.py` - Simple text memory (no external dependencies)

### Multimodal (`examples/multimodal/`)
- ✅ `text.py` - Text memory with semantic search (no external dependencies)
- 🔑 `images.py` - Image analysis and search (requires OpenAI API key)
- 🔑 `video.py` - Video frame and audio analysis (requires OpenAI API key) 
- 🔑 `audio.py` - Audio transcription and search (requires Whisper)
- `pdf.py` - Document processing and search
- `website.py` - Web content extraction and search

### Integrations (`examples/integrations/`)
- 🔑 `langchain_chat_history.py` - LangChain persistent chat (requires OpenAI API key)
- 🔑 `langchain_financial_rag_agent.py` - Financial document RAG (requires OpenAI API key)
- `langchain_multimodal_memory.py` - Multimodal LangChain integration
- 📦 `crewai_agentic_rag.py` - CrewAI knowledge search (requires `pip install crewai`)

### Services (`examples/fastapi/`, `examples/mcp/`)
- 📦 `fastapi/memory_service.py` - REST API service (requires `pip install fastapi uvicorn`)
- 📦 `mcp/memory.py` - Model Context Protocol server (requires MCP dependencies)

### Environment Setup

**Working without external dependencies:**
```bash
python examples/getting_started/01_basic_memory.py
python examples/multimodal/text.py
```

**For OpenAI-powered examples (🔑):**
```bash
export OPENAI_API_KEY="your-api-key-here"
python examples/multimodal/images.py
```

**For additional integrations (📦):**
```bash
pip install crewai  # For CrewAI examples
pip install fastapi uvicorn  # For FastAPI service
```
...
# Define memory structure using context-based API
context = [
    Text(id="user_id", embed=False),  # Metadata only
    Text(id="role", embed=False),     # Metadata only  
...
def chat_with_ai_and_memories(message: str, user_id: str = "default_user") -> str:
    ...
    # Store memories using the Entry dataclass
    user_entry = chat_memory.Entry(user_id=user_id, role="user", content=message)
    assistant_entry = chat_memory.Entry(user_id=user_id, role="assistant", content=assistant_response)
    chat_memory.add(user_entry, assistant_entry)
...
# Define context using the new API
context = [
    Text(id="memory_id", embed=False),
    Text(id="role", embed=False), 
    Text(id="content", embed=True, embed_model="intfloat/e5-large-v2"),
    Text(id="insert_at", embed=False),
]
...
# Define context with images using the context-based API
context = [
    Image(id="image", provider="openai", model="gpt-4o-mini", 
          prompt="Describe this image in detail, focusing on objects, colors, and scene."),
    Text(id="caption", embed=True),
    Text(id="tags", embed=False),
]
...
context = [
    Image(id="image", provider="openai", model="gpt-4o-mini",
          prompt="Analyze this image: 1) Objects and colors 2) Setting 3) Text visible 4) Mood",
          llm_kwargs={"max_tokens": 300, "temperature": 0.3},
          use_clip=True, clip_model="openai/clip-vit-base-patch32"),
    Text(id="title", embed=False)
]
...
sunset_entry = memory.Entry(image="sunset.jpg", title="Beach Sunset")
city_entry = memory.Entry(image="city.jpg", title="Urban Skyline")
memory.add(sunset_entry, city_entry)
...
context = [
    Text(id="content_type", embed=False),
    Image(id="image", prompt="Describe this image for search: key objects, colors, setting, and context."),
...
image_entry = memory.Entry(
    content_type="image",
    image="garden.jpg", 
    text_content=None,
    title="Garden Photo"
)
text_entry = memory.Entry(
    content_type="text",
    image=None,
    text_content="Guide to growing colorful flowers in small spaces",
    title="Gardening Tips"
)
memory.add(image_entry, text_entry)
...
```python
from pixelmemory.context import Image

# OpenAI GPT-4o (default)
openai_context = [Image(id="image", provider="openai", model="gpt-4o-mini", 
                        llm_kwargs={"max_tokens": 200})]
openai_memory = Memory(context=openai_context, namespace="openai_vision")

# Google Gemini  
gemini_context = [Image(id="image", provider="gemini", model="gemini-2.0-flash",
                        llm_kwargs={"config": {"temperature": 0.4, "max_output_tokens": 300}})]
gemini_memory = Memory(context=gemini_context, namespace="gemini_vision")

# Anthropic Claude
anthropic_context = [Image(id="image", provider="anthropic", model="claude-3-haiku-20240307")]
anthropic_memory = Memory(context=anthropic_context, namespace="anthropic_vision")
```
...
## 🔗 Next Steps: Explore the Pixeltable Ecosystem

PixelMemory demonstrates just one way to leverage Pixeltable's powerful capabilities. To build more sophisticated agents and AI applications:

- **[Learn Pixeltable fundamentals](https://docs.pixeltable.com/docs/overview/quick-start)** - Master the underlying platform
- **[Read our stateful agents guide](https://www.pixeltable.com/blog/building-memory-powered-ai-stateful-agents-pixeltable)** - Deep dive into memory architectures
- **[Explore Pixeltable examples](https://docs.pixeltable.com/docs/examples/use-cases)** - See advanced patterns and use cases
- **[Join the community](https://discord.gg/QPyqFYx2UN)** - Get help and share your own memory implementations

**Remember**: PixelMemory is a reference implementation. Feel free to adapt, extend, or rebuild it to match your specific agent memory requirements using Pixeltable's flexible primitives.
